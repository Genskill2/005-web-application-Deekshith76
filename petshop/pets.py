...
def search(field, value):
    ...
    if order == "asc":
        cursor.execute(f"select p.id, p.name, p.bought, p.sold, s.name from pet p, animal s, tag t, tags_pets tp where t.name = ? and p.species = s.id and tp.pet = p.id and tp.tag = t.id order by p.{oby}", [value])
    else:
        cursor.execute(f"select p.id, p.name, p.bought, p.sold, s.name from pet p, animal s, tag t, tags_pets tp where t.name = ? and p.species = s.id and tp.pet = p.id and tp.tag = t.id order by p.{oby} desc", [value])
